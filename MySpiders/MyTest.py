...
def xiecheng(dcity, acity, date):
    ...
    for route in routeList:
        if len(route.get('legs')) == 1:
            ...
            row = []
            row.append(flight.get('airlineName'))
            row.append(flight.get('flightNumber'))
            row.append(flight.get('departureDate')[-8:-3])
            row.append(flight.get('arrivalDate')[-8:-3])
            row.append(flight.get('punctualityRate'))
            row.append(legs.get('characteristic').get('lowestPrice'))
            table.add_row(row)
...
if __name__ == "__main__":
    dcity = raw_input('From:')
    acity = raw_input('To:')
    date = raw_input('请输入出行日期：')
